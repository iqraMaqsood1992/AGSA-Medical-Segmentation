...
def main():
    ...
    # Add these near the top of your train.py where you define the argument parser
    parser.add_argument('--pre_trained', action='store_true', help='use pre-trained model')
    parser.add_argument('--load_path', default=None, type=str, help='path to checkpoint to load')
    ...
    # Use command-line args instead of opt for loading pre-trained
    checkpoint_loaded = False
    if args.pre_trained and args.load_path:
        print(f"Loading checkpoint from: {args.load_path}")
        try:
            model.load_state_dict(torch.load(args.load_path))
            checkpoint_loaded = True
            print(f"Successfully loaded checkpoint - starting from epoch {args.start_epoch}")
    ...
    best_dice = 0.0 
    if checkpoint_loaded and args.start_epoch > 0:
        # Extract dice score from checkpoint filename if possible
        try:
            dice_str = args.load_path.split('_')[-1].replace('.pth', '')
            best_dice = float(dice_str)
            print(f"Continuing with previous best dice: {best_dice}")
        except:
            print("Could not extract previous best dice from filename")
    
    loss_log = np.zeros(opt.epochs+1)
    
    # Start from the specified epoch
    start_epoch = args.start_epoch if args.start_epoch > 0 and checkpoint_loaded else 0
    for epoch in range(start_epoch, opt.epochs):
        ...
        for batch_idx, (input_image, ground_truth, mask_mini, *rest) in enumerate(trainloader):
            ...
            # Combined loss with attention-enhanced components
