...
try:
    from models.attention import SynapseAttention
except ImportError:
    # Fallback to vitAttention if SynapseAttention is not available
    pass
...
class PreNorm2in(nn.Module):
    def __init__(self, dim, fn):
        ...
        self.fn = fn
    def forward(self, x1, x2, attention_weights=None):
        if attention_weights is not None:
            return self.fn(self.norm1(x1), self.norm2(x2), attention_weights)
        else:
...
class qkvAttention(nn.Module):
    ...
    def forward(self, q, kv, weights=None):
        B, Nq, _ = q.shape
        Nk = kv.shape[1]
        ...
        if weights is not None:
            # Process safely to avoid dimension mismatch
            pass
...
class DSTransformerIP(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim=1024, dropout=0., n_classes=2, n_cprototypes=64, f_merge=3, input_size=(32, 32), pdepth=2, k_class0=1):
        ...
        # Use vitAttention if SynapseAttention is not available
        try:
            from models.attention import SynapseAttention
            attention_class = SynapseAttention
        except ImportError:
            attention_class = vitAttention
        
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm2in(dim, attention_class(dim, heads=heads, dim_head=dim_head, dropout=dropout)),
    ...
    def forward(self, x):
        ...
        # Fix: Don't add x to prototypes before passing to pattn
        ...
        for id_layer, (attn, ff, dm) in enumerate(self.layers):
            ...
            # Fix: Don't add prototypes to x before passing to attn
